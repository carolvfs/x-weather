...
class Map {
    constructor(element, lat, lon, event, selectedPoints, updateHmat, update3rdChart, updateSelection, 
        ...
        vis.renderCities()
        vis.buildLeg(vis.globalLegGroup, 'linear-gradient', mapIndex, vis.yGlobalLeg, vis.axisGlobalLeg, vis.globalOffset, 26, vis.globalUnit, 25)
    ...
    buildLeg(group, gradientName, id, yScale, axisLeg, offset, xRect, unit, xTitle) {
        const vis = this
    ...
    updateLeg(color, domain, yScale, axisLeg, axisLegGroup) {
        const vis = this
    ...
    update(event, selectedPoints, mapIndex, globalData, globalDomain, lensData, lensDomain) {
        ...
        vis.updateContours()
        vis.updateLeg(vis.globalColor, vis.globalDomain, vis.yGlobalLeg, vis.axisGlobalLeg, vis.axisGlobalLegGroup)
        ...
        if(event === 'brush') {
            ...
            if(vis.lensControl === false) {
                vis.buildLeg(vis.lensLegGroup, 'lens-linear-gradient', vis.mapIndex, vis.yLensLeg, vis.axisLensLeg, vis.lensOffset, vis.width-50-8, vis.lensUnit, vis.width-50)
                vis.updateLeg(vis.lensColor, vis.lensDomain, vis.yLensLeg, vis.axisLensLeg, vis.axisLensLegGroup)
